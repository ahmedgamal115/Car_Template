  ...
  return (
    <div className={``}>
      ...
      onMouseLeave={()=>{setBtnHover(false);}}>
        <PiOctagonThin  className="w-[70px] h-[70px]
        fill-slate-500 z-20"/>
        <PiOctagonFill className={`w-[70px] h-[70px] z-10
          absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]
          ${btnHover ? 'fill-master-blue' : 'fill-transparent'}`}/>
        {
          dirction === 'left' 
          ? <FaAngleLeft className="w-[35px] h-[35px] z-10 text-slate-100
          absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]"/>
          : <FaAngleRight className="w-[35px] h-[35px] z-10 text-slate-100
          absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]"/>
        }
      </div>
    </div>
